...
<div class="notice--warning">
  <h4>Please note</h4>
  <p>It might take a couple of seconds before the mathematical notation is correctly loaded into your web browsers.</p>
</div>
...
To solve the kernelized SVM problem, we apply the so-called Sequential Minimal Optimization algorithm (or, abbreviated, "SMO"). The SMO breaks the large QP problem into a series of smallest possible QP problems. These small QP problems are solved analytically, which avoids using a time-consuming numerical QP optimization as an inner loop. At every step, SMO chooses two Lagrange multipliers to jointly optimize, finds the optimal values for these multipliers, and updates the SVM to reflect the new optimal values. 

Without loss of generality, let the two multipliers we try to optimize be \\(\alpha_1\\) and \\(\alpha_2\\). When we optimize two variables at a time, the objective function can be obtained by dropping all constant terms independent of the two selected variables \\(\alpha_1\\) and \\(\alpha_2\\) in the original objective function:
...
\textrm{subject to} & \alpha_{1} y^{(1)} + \alpha_2 y^{(2)} = - \sum_{i \neq 1,2}^{m} \alpha_{i} y^{(i)} \newline
& 0 \leq a_1, a_2 \leq C
\end{array}
\end{equation}
...
where \\(\rho_{k} = \sum_{i}^{m} \alpha_i y^{(i)} K_{ik} +b\\) is the output of the kernelized decision function of Equation \eqref{4-5} with \\(x^{(k)}\\) as the input value. Note that \\(\upsilon_{k}\\) is independent of the update of the Lagrangian multipliers \\(\alpha_{1}\\) and \\(\alpha_{2}\\). That is, we can express \\(\upsilon_{k}\\) and \\(rho_{k}\\) using only old values of alpha. From this, it follows that \\(rho_{k} = \sum_{i}^{m} \alpha_{i}^{old} y^{(i)} K_{ik} + b\\). 

Using Equation \eqref{5-5}, the objective function can be simplified to:
...
\begin{align}
L(\alpha_2) & = [\zeta - s \alpha_2] + \alpha_2 - \frac{1}{2}  ([\zeta - s \alpha_2]^2 K_{11} +   \alpha_2^{2} K_{22} + 2 [\zeta - s \alpha_2] \alpha_{2}y^{(1)}y^{(2)}K_{12}) \tag{5-7} \label{5-7} \newline
&\quad - [\zeta - s \alpha_2]  y^{(1)} \upsilon_1 - \alpha_2 y^{(2)} \upsilon_2 \newline
& = \zeta + \alpha_2 (1 - s)  - \frac{1}{2}  [\zeta - s \alpha_2]^2 K_{11} - \frac{1}{2}  \alpha_2^{2} K_{22} - [\zeta - s \alpha_2] \alpha_{2} s K_{12} \newline
&\quad - [\zeta - s \alpha_2]  y^{(1)} \upsilon_1 - \alpha_2 y^{(2)} \upsilon_2 \newline
& = \zeta + \alpha_2 (1 - s)  - \frac{1}{2} (\zeta - s \alpha_{2})(\zeta - s \alpha_{2}) K_{11} - \frac{1}{2} \alpha_{2}^{2} K_{22} - \zeta \alpha_{2} s K_{12} + \alpha_{2}^{2} s^{2} K_{12} \newline
&\quad - \zeta y^{(1)} \upsilon_{1} + s \alpha_{2} y^{(1)} \upsilon_{1} - \alpha_{2} y^{(2)} \upsilon_{2} \newline
& = \zeta + \alpha_{2}(1-s) - \frac{1}{2} \zeta^{2} K_{11} + s \alpha_{2} \zeta K_{11} - \frac{1}{2} s^{2} \alpha_{2}^2 K_{11} - \frac{1}{2} \alpha_{2}^{2} K_{22} - \zeta \alpha_{2} s K_{12} + \alpha_{2}^{2} s^{2} K_{12} \newline
&\quad - \zeta y^{(1)} \upsilon_{1} + s \alpha_{2} y^{(1)} \upsilon_{1} - \alpha_{2} y^{(2)} \upsilon_{2} \newline
& =  (1 - s + \zeta s K_{11} -\zeta s K_{12} + s y^{(1)}\upsilon_{1} - y^{(2)}\upsilon_{2}) \alpha_{2} + \frac{1}{2} (-s^{2} K_{11} - K_{22} + 2s^{2}K_{12}) \alpha_{2}^{2} \newline
&\quad - (\frac{1}{2} \zeta^{2} K_{11} + \zeta y^{(1)} \upsilon_1) \newline
& =  (1 - s +  \zeta s K_{11} -\zeta s K_{12} + y^{(1)} y^{(2)} y^{(1)}\upsilon_{1} - y^{(2)}\upsilon_{2}) \alpha_{2} + \frac{1}{2} (-s^{2} K_{11} - K_{22} + 2s^{2}K_{12}) \alpha_{2}^{2} + c \newline
& =  (1 - s + \zeta s (K_{11} - K_{12}) +  y^{(2)} (\upsilon_{1} - \upsilon_{2})) \alpha_{2} + \frac{1}{2} (2K_{12} - K_{11} - K_{22}) \alpha_{2}^{2} + c
\end{align}
...
\begin{align}
\eta &= 2K_{12} - K_{11} - K_{22} \Leftrightarrow 2z_{1}^Tz_{2} - z_{1}^Tz_{1} - z_{2}^Tz_{2} \Leftrightarrow -(z_{1}^T-z_{2}^{T}) (z_{1} - z_{2}) \Leftrightarrow \tag{5-9} \label{5-9} \newline
&\quad  -(z_{1}-z_{2})^T(z_{1}-z{2}) \Leftrightarrow - \|z_{1}-z_{2}\|^{2} \leq 0 
\end{align}
...
&\quad{} = (y^{(2)})^2 - y^{(1)}y^{(2)} +  y^{(2)} (\rho_{1} - \rho_{2}) - \alpha_{2} \eta \newline
&\quad{} = y^{(2)} (\rho_{1} - y^{(1)} - (\rho_{2} -y^{(2)})) - \alpha_{2} \eta \newline
...
\begin{align}
\frac{\delta}{\delta \alpha_2} L{\alpha_2} = 0 \Leftrightarrow \eta \alpha_{2} + y^{(2)} (E_{1} - E_{2}) - \alpha_{2}^{old} \eta = 0 \Leftrightarrow \alpha_{2}^{new} =  \alpha_{2}^{old} - \frac{y^{(2)} (E_{1} - E_{2})}{\eta} \tag{5-14} \label{5-14}
\end{align}

where \alpha_{2}^{new} denotes the unconstrained maximum of \\(\alpha_{2}\\). 

#### 5.2 Parameter clipping
The optimum values of alpha that we found in the previous section are unconstrained. That is, they do not necessarily satisfiy the constraints imposed on them in Equation \eqref{5-2}. To assure that the alphas satisfiy these constraints, we clip its values. 

Recall that the linear equality constraint in Equation \eqref{5-2} requires the Lagrange multipliers to lie on a diagonal line. Furthermore, the bound constraints in Equation \eqref{5-2} requires the Lagrange multipliers to lie within the box \\([0,C] \text{x} [0,C]\\). Thus, both constraints imply that the constrained maximum of the objective function must lie on a diagonal line segment. Overall, there will be some lower-bound \\(L\\) and some upper-bound \\(U\\) on the permissible values for \\(\alpha_{2}\\) that will ensure that \\(\alpha_{1}\\) and \\(\alpha_{2}\\) lie on this diagonal line segment. In particular,


* If \\(s = 1\\), i.e. the line is downward sloping, then \\(\alpha_1 + s\alpha_2 = \alpha_1 + \alpha_2 = \zeta \Leftrightarrow \alpha_2 = \zeta - \alpha_1\\). Note that we can interpret \\(\zeta\\) as the bias of this linear function.
   * If \\(0 < \zeta < C\\) then \\(min(\alpha_2)\\) is at \\(\alpha_{2} = 0\\) and  \\(max(\alpha_{2})\\) is at \\(\alpha_1 = 0 \Rightarrow \alpha_2 = \zeta\\). 
   * If \\(\zeta > C\\) then \\(min(\alpha_2)\\) is at \\(\alpha_1 = C \Rightarrow \alpha_2 = \zeta - C\\) and \\(max(\alpha_2)\\) is at \\(\alpha_2 = C\\). 
...
\tag{5-15} \label{5-15}
\begin{cases}
L = max(0, \zeta - C) = max(0, \alpha_{1} + \alpha_{2}) \newline
U = min(\zeta, C) = min(\alpha_{1} + \alpha_{2}, C) \newline
\end{cases}
\end{align}

* If \\(s = -1\\), i.e. the line is upward sloping, then \\(\alpha_1 + s\alpha_2 = \alpha_1 - \alpha_2 = \zeta \Leftrightarrow \alpha_2 = -\zeta + \alpha_1\\). Note that we can again interpret \\(\zeta\\) as the bias of this linear function.
   * If \\(0 < \zeta < C\\) then \\(min(\alpha_2)\\) is at \\(\alpha_2=0\\) and  \\(max(\alpha_2)\\) is at \\(\alpha_1 = C \Rightarrow \alpha_2 = -\zeta + C\\). 
   * If \\(\zeta < 0\\) then \\(min(\alpha_{2})\\) is at \\(\alpha_1 = 0 \Rightarrow alpha_2 = -\zeta\\) and \\(max(\alpha_2)\\) is at \\(\alpha_2 = C\\).
...
U = min(C - \zeta, C) = min(C - \alpha_1 + \alpha_2, C) \newline
\end{cases}
\end{align}
...
\begin{align}
\alpha^{new, clipped}_2 = \tag{5-17} \label{5-17}
\begin{cases}
H &\quad{} \text{if } \alpha_{2}^{new} \geq H \newline
\alpha_{2}^{new} &\quad{} \text{if } L < \alpha_{2}^{new} < H \newline
L &\quad{} \text{if } \alpha_{2}^{new} \leq L \newline
\end{cases}
\end{align}

Finally, we can use \\(\alpha_1^{new} = \alpha_1^{old} + s (\alpha_{2}^{old} - \alpha_{2}^{new, clipped})\\) from our discussion surrounding \eqref{5-4} to find the maximum of \\(\alpha_{1}\\).
...
Using the error term as used in Equation \eqref{5-10}, we consider the following equality,

\begin{align}
E_{k}^{new} - E_{k}^{old} & =  (\rho_{k}^{new} - y^{(k)}) -  (\rho_{k}^{old} - y^{(k)}) \tag{5-18} \label{5-18} \newline
&\quad =  \rho_{k}^{new} - \rho_{k}^{old}\newline
&\quad = (\sum_{i=1}^{m} \alpha_{i}^{new} y^{(i)} K_{ik} + b^{new}) - (\sum_{i=1}^{m} \alpha_{i}^{old} y^{(i)} K_{ik} + b^{old}) \newline
&\quad = \sum_{i=1}^{m} a_{i}^{new} y^{(i)} K_{ik} - \sum_{i=1}^{m} a_{i}^{old} y^{(i)} K_{ik} + b^{new} - b^{old} \newline
&\quad = \sum_{i\neq 1,2}^{m} \alpha_{i}^{new} y^{(i)} K_{ik} + \sum_{i = 1,2} a_{i}^{new} y^{(i)} K_{ik} - \sum_{i\neq 1,2}^{m} \alpha_{i}^{old} y^{(i)} K_{ik} 
- \sum_{i = 1,2} a_{i}^{old} y^{(i)} K_{ik} + b^{new} - b^{old} \newline
&\quad = \sum_{i = 1,2} a_{i}^{new} y^{(i)} K_{ik} - \sum_{i = 1,2} \alpha_{i}^{old} y^{(i)} K_{ik} + b^{new} - b^{old} \newline
&\quad = (\alpha_{1}^{new} y^{(1)} K_{1k} + \alpha_{2}^{new} y^{(2)} K_{2k}) - (\alpha_{1}^{old} y^{(1)} K_{1k} + \alpha_{2}^{old} y^{(2)} K_{2k}) + b^{new} - b^{old} \newline
&\quad = (a_{1}^{new} - \alpha_{1}^{old}) y^{(1)} K_{1k} + (\alpha_{2}^{new, clipped} - \alpha_{2}^{old}) y^{(2)} K_{2k} + b^{new} - b^{old} \newline
\end{align}
...
* If \\(0 < \alpha_{k} < C\\) for k = 1 or k = 2, then \\(y^{(k)} E_{k}\\) should  equal 0 according to the KKT conditions.\* Since \\(y^{(k)} \in \\{-1, 1\\}\\), we force \\(E_{k}^{new}\\) to equal zero for this to hold true. Substituting this into Equation \eqref{5-18} yields,

\begin{align}
0 - E_{k}^{old} & = (\alpha_{1}^{new} - \alpha_{1}^{old}) y^{(1)} K_{1k} + (\alpha_{2}^{new} - \alpha_{2}^{old}) y^{(2)} K_{2k} + b^{new}_{k} - b_{k}^{old} \quad{} (k = 1 \text{or } 2) \tag{5-19} \label{5-19}\newline
b^{new}_k & = - E_{k}^{old} - (a_{1}^{new} - a_{1}^{old}) y^{(1)} K_{1k} - (a_{2}^{new} - a_{2}^{old}) y^{(2)} K_{2k} + b_{k}^{old} \newline
\end{align}

Note that, if both both \\(0 < \alpha_1 < C\\) and \\(0 < \alpha_2 < C\\), the optimal threshold values are are equal.

---- UNFINISHED -----

### 5.4 Python implementation 
The python code for the main part of the SMO algorithm is listed below.

```python

# Get training data in an m by n+1 matrix form, where m denotes the number of 
# examples and n denotes the number of features and the '+1' refers to the 
# labels of each training example

import numpy as np

X, Y = X_values, Y_values
m, n = X.shape[0], X.shape[1]   # data size
alpha = np.zeros((m,1))         # alpha variables
bias = 0                        # initialize bias term
iteration = 0                   # interation counter

max_iter = 10000                # maximum number of iterations 
C = 1.0                         # penalty term
epsilon = 0.001                 # tolerance for KKT violation             

while iteration < maxiteration:
    iteration += 1              # add one to number of iters
    alphas_changed = 0          # number of alphas changed
    for i in range(1, m+1):     # for each alpha_i
        assert np.sum(np.dot(X, X[i,:].T)) == X * X[i,:]
        Ei = (alpha * Y) * (X * X[i,:]) + b - Y[i,:]
        yE = Y[i] * Ei
        if 0 < alpha[i] < C and -epsilon < yE < epsilon: # get alphas which violate KKT conditions
            continue
        for j in range (1, m+1): # for each alpha_j
            if j == i:           # take an alpha_j =/= alpha_j
                continue
            ...
            if L==H:             # skip when L==H, i.e. straight line
                continue
            xi, xj = X[i,:], X[j,:]
            eta = 2 * (xi * xj) - (xi * xi) - (xj * xj)
            alpha[j,:] = aj + Y[j,:] * (Ej - Ei)/eta   # update alpha_j
            if alpha[j,:] > H:
                alpha[j,:] = H
            elif aj_new < L:
                alpha[j,:] = L
            if np.linalg.norm(alpha[j,:]-aj) < tol   # next alpha if change is too small
                continue
            alpha[i,:] = ai - Y[i,:] * Y[j,:] * (alpha(j,:) - aj) # calculate optimal value of alpha_i
            bi = bias - Ei - Y[i,:] * (alpha[i,:]-ai) *  (X[i,:] * X[i,:]) - Y[j,:] * (alpha[j,:] - aj) * (X[i,:] * X[j,:])
            bi = bias - Ej - Y[i,:] * (alpha[i,:]-ai) *  (X[i,:] * X[j,:]) - Y[j,:] * (alpha[j,:] - aj) * (X[j,:] * X[j,:])
            if 0 < alpha[i,:] < C:
                bias = bi
            elif 0 < alpha[j,:] < C:
                bias = bj
