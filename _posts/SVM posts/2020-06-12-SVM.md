...
Recall that the linear equality constraint in Equation \eqref{5-2} requires the Lagrange multipliers to lie on the diagonal line \\(\alpha_{1} + s\alpha_{2} = \zeta\\). Furthermore, the bound constraints in Equation \eqref{5-2} requires the Lagrange multipliers to lie within the box \\([0,C] \text{ x } [0,C]\\). Thus, both constraints imply that the constrained maximum of the objective function must lie on a diagonal line segment. 

Overall, there will be some lower-bound \\(L\\) and some upper-bound \\(U\\) on the permissible values for \\(\alpha_{2}\\) that will ensure that \\(\alpha_{1}\\) and \\(\alpha_{2}\\) lie on this diagonal line segment. Note that we can interpret \\(\zeta\\) as the bias term of the linear function.

As demonstrated in Figure 6 and 7, we can distinguish between four cases. 

__Figure 6: Lower and upperbound if \\(s = 1\\), i.e. the line is downward sloping__

![Figure 6: Lower and upperbound 1](/images/sequals1.png)

If \\(s = 1\\) (Fig 6), i.e. the line is downward sloping, then \\(\alpha_1 + s\alpha_2 = \zeta \Rightarrow \alpha_2 = \zeta - \alpha_1\\). Moreover,
...
__Figure 7: Lower and upperbound if \\(s = -1\\), i.e. the line is upward sloping__
