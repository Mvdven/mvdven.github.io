...
As demonstrated in Figure 6, if \\(s = 1\\), i.e. the line is downward sloping, then \\(\alpha_1 + s\alpha_2 = \zeta \Rightarrow \alpha_2 = \zeta - \alpha_1\\). Moreover,
...
On the other hand, it is demonstrated in Figure 7 that if \\(s = -1\\), i.e. the line is upward sloping, then \\(\alpha_1 + s\alpha_2 = \zeta \Rightarrow \alpha_2 = -\zeta + \alpha_1\\). Moreover,
...
* _Note that \\(y^{(k)} E_{k} = 0 \Leftrightarrow y^{(k)} (\sum_{i=1}^{m} \alpha_{i} y^{(i)} K_{ik} + b - y^{(k)}) = 0 \Leftrightarrow y^{(k)} f(x^{(k)}) - 1 = 0\\)_ 
...
# examples, n denotes the number of features and the '+1' refers to the 
...
while iteration < maxiteration:
    ...
    for i in range(1, m+1):     # for each alpha_i
        ...
        for j in range (1, m+1): # for each alpha_j
            ...
            if eta >= 0:		# Optimization does not assure maximum, thus next alphas
            	continue
