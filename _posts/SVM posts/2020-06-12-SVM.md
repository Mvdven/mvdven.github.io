...
Overall, there will be some lower-bound \\(L\\) and some upper-bound \\(U\\) on the permissible values for \\(\alpha_{2}\\) that will ensure that \\(\alpha_{1}\\) and \\(\alpha_{2}\\) lie on this diagonal line segment. As demonstrated in Figure 6 and 7, we can distinguish between four cases. 
