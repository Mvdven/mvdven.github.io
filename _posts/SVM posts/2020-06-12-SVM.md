...
<font size="6"><b>  Chapter 4: Kernelization </b></font>
---

 
### 4.1 Kernel-trick
Kernel functions can be regarded as a generalized dot product between two vectors \\(x\\) and \\(y\\) in some (possible very) high dimensional feature space, in which our data is more likely to be linearly seperatable.

__Figure 5: Linear separability in higher dimensional space__

![Figure 5: Example of Kernel mapping](/images/simple-example-of-kernel.png)
...
\begin{align}
K(x^{(i)}, x^{(j)}) = (\phi(x)^{T} \phi(y) \tag{4-1} \label{4-1}
\end{align}
...
### 4.2 Example
Consider the simple polynomial kernel \\(K(x, y) = (1+x^(T)z)^{2}\\). Without loss of generality, we assume that \\(x, y \in \mathcal{R}^{2}\\). At first glance, this does not seem to correspond to any mapping function \\(\phi\\). Assuming that \\(x = (x_{1}, x_{2})\\) and \\(y = (y_{1}, y_{2})\\), we can expand the expression as,

\\(K(x, y) &= (1+x^{T}y)^{2} \Leftrightarrow (1+x_{1}y_{1} + x_{2}y_{2})^{2} \tag{4-2} \label{4-2} \newline
 &\quad{} = 1 + x_{1}^{2} y_{1}^{2} + x_{2}^{2} y_{2}^{2} + 2x_{1}y_{1} + 2x_{2}y_{2} + 2x_{1}x_{2}y_{1}y_{2}\\) 

Note that Equation \eqref{4-2} is nothing else but the dot product between the vectors
...
So, the Kernel function \\(K(x, y) = (1+x^(T)z)^{2} = \phi(x)^T \phi(y)\\)computes a dot product in 6-dimensional space without explicity setting foot in it. 

Other examples include the exponential kernel, the radial basis function, and the most popular Gaussian kernel \\(k(x,y) = \text{exp} (- \gamma \|x-y\|^2)\\), which corresponds to an infinite-dimensional mapping. 

### 4.3 Kernelized SVM
The kernel-trick can be applied to the SVM algorithm considered previously because all the data points appear in the algorithm by means of an inner product.  To apply the kernel trick, during the training process, we replace the inner product \\(\langle x^{(i)}, x^{(j)} \rangle \\) by a Kernel function \\(K(x^{(i)}, x^{(j)})\\) to get,

\begin{equation}
\begin{array}{rrclcl}
\displaystyle \max_{\alpha} & \sum_{i=1}^{m} \alpha_i - \frac{1}{2} \sum_{i,j=1}^{m} \alpha_i \alpha_j y^{(i)}y^{(j)} K(x^{(i)}, x^{(j)} \tag{4-2} \label{4-2} \newline
\textrm{subject to} &0 \leq \alpha_i \leq C \quad{} \forall i = 1, ..., m \newline
& \sum_{i=1}^{m}\alpha_iy^{(i)} = 0  \newline
\end{array}
\end{equation}

Similarly, after finding the optimal alpha values, we classify new examples using,

\begin{align}
f(x) = \sum_{i=1}^{m} y^{(i)} \alpha_{i}^{\*} \langle x^{(i)}, x \rangle + b^{\*} \Leftrightarrow \sum_{i=1}^{m} y^{(i)} \alpha_{i}^{\*} K(x_{i}, x) + b^{\*} \tag{4-3} \label{4-3}
\end{align}

where \\(b^{\*}\\) is defined as

\begin{align}
b^{\*} = \frac{1}{\|S\|} \sum_{i \in S}  [y^{(i)} - \sum_{j=1}^{m} \alpha_j^{\*} y^{(j)} K(x^{(j)}, x^{(i)})] \tag{4-4} \label{4-4}
\end{align}
