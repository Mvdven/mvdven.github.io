...
Similarly, we can simplify the coefficient of the linear term \\(\alpha_{2}\\). In particular, using \\(\zeta = \alpha_1^{old} + s\alpha_2^{old}\\) and \\(\upsilon_k\\) from the discussions surrounding Equation \eqref{5-4} and \eqref{5-5}, respectively, the coefficient of \\(\alpha_2\\) is given by,
...
### 5.4 Implementation in Python (linear kernel) 
