...
### 4.2 Example
Consider the simple polynomial kernel \\(K(x, y) = (1+x^(T)z)^{2}\\). At first glance, this does not seem to correspond to any mapping function \\(\phi\\). Without loss of generality, we assume that \\(x, y \in \mathcal{R}^{2}\\). Furthermore, \\(x = (x_{1}, x_{2})\\) and \\(y = (y_{1}, y_{2})\\). This allows us to expand the expression as
...
Similarly, after finding the optimal alpha values, we classify new examples by replacing the inner product inside Equation \eqref{3.15} with the kernel function,

\begin{align}
f(x) = \sum_{i=1}^{m} y^{(i)} \alpha_{i}^{\*} k(x_{i}, x) + b^{\*} \tag{4-3} \label{4-3}
\end{align}
