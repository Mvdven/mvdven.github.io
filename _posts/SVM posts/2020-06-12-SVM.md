...
From a numerical stability standpoint, and in particular when taking into account the stopping criteria, the actual value of the margin may not be exactly equal to 1, so generally one averages over the \\(b^{\*}\\)'s of all support vectors when performing the calculation. If we denote \\(S = \\{i|a_{i}^{\*}>0\\}\\) as the set of support vectors, then
