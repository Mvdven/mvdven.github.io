...
So, the kernel function \\(k(x, y) = (1+x^(T)z)^{2} \Leftrightarrow \phi(x)^T \phi(y))\\) computes a dot product in 6-dimensional space without explicity setting foot in it.
