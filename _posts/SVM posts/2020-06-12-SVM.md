...
\begin{align} 
L(w, b, \xi, \alpha, \beta) = \underbrace{\frac{1}{2}\|w\|^{2} + C \sum_{i}^{m} \xi_{i}}_\textrm{Primal objective} + \underbrace{\sum_{i}^{m} \alpha_i [(1 - \xi_{i}) - y^{(i)} (w^{T}x^{(i)} + b)]}_\textrm{First set of ineq. constraints} + \underbrace{\sum_{i}^{m} \beta_{i} (-\xi_{i})}_\textrm{Second set of ineq. constraints} \tag{3-1} \label{3-1}
\end{align}
...
\begin{align}
\min_{w, b, \xi} \theta_P(w,b, \xi)  =  \min_{w, b, \xi} \max_{\substack{\alpha, \beta \\ \alpha_i \geq 0, \beta_i \geq 0}} L(w, b, \xi, \alpha, \beta)  \tag{3-4} \label{3-5}
\end{align}
...
\begin{align}
\theta_D(\alpha,\beta):  \min_{w, b, \xi} L(w, b, \xi, \alpha, \beta) \tag{3-5} \label{3-5}\\
\end{align}

where the subscript \\(D\\) stands for dual. Then the dual optimization problem is given by

\begin{align}
\max_{\substack{\alpha,\beta \\ \alpha_i \geq 0, \beta_i \geq 0}} \theta_D(\alpha,\beta) = \max_{\substack{\alpha,\beta \\ \alpha_i \geq 0, \beta_i \geq 0}} \min_{w, b, \xi} L(w, b, \xi, \alpha, \beta) \tag{3-6} \label{3-6} \\
\end{align}
...
\begin{align}
\max_{\substack{\alpha,\beta \\ \alpha_i \geq 0, \beta_i \geq 0}} \theta_D(\alpha,\beta) \leq  \min_{w, b} \theta_P(w,b) \tag{3-7} label{3-7}\\
\end{align}
...
### 3.2 KKT Conditions and strong duality
...
1. Primal feasibility: the primal constraints hold, i.e.
    ...
    1.1 \\(\quad{} y^{(i)} ((w^{\*})^{T}x^{(i)} + b^{\*}) \geq 1 - \xi_i^{\*}  \quad{} \forall i = 1, ..., m \label{1.1}\\)
    1.2 \\(\quad{} \xi_i^{\*} \geq 0 \quad{} \forall i = 1, ..., m \label{1.2}\\)


2. Dual feasibilitiy: the dual constraints hold, i.e.
    
    2.1 \\(\quad{} a^{\*} \geq 0 \label{2.1} \\)
    2.2 \\(\quad{} \beta^{\*} \geq 0 \label{2.2}\\)


3. Complimentary slackness, i.e.
    
    3.1 \\(\quad{} \alpha^{\*}_{i} [(1 - \xi_{i}^{\*}) - y^{(i)} ((w^{\*})^{T}x^{(i)} + b^{\*})] = 0 \quad{} \forall i = 1, ..., m \label{3.1}\\)
    3.2 \\(\quad{} \beta^{\*}_{i} (- \xi_{i}^{\*}) = 0 \quad{} \forall i = 1, ..., m \label{3.2}\\)

4. Stationarity: the gradient with respect to \\(w\\), \\(b\\) and \\(\xi\\) is zero, i.e.

    4.1 \\(\quad{} \frac{\delta}{\delta w}L(w^{\*}, b^{\*}, \xi^{\*}, \alpha^{\*}, \beta^{\*}) = 0 \label{4.1} \\)
    4.2 \\(\quad{} \frac{\delta}{\delta b}L(w^{\*}, b^{\*}, \xi^{\*}, \alpha^{\*}, \beta^{\*}) = 0 \label{4.2}\\)
    4.3 \\(\quad{} \frac{\delta}{\delta \xi}L(w^{\*}, b^{\*}, \xi^{\*}, \alpha^{\*}, \beta^{\*}) = 0 \label{4.3}\\)

From these conditions, we can extract some usefull information. In particular,

* If \\(\alpha_{i}^{\*} = 0 \quad{} \xrightarrow{\text{Eq.} \eqref{5-11}} \quad{}  \beta_{i}^{\*} = C \quad{} \xrightarrow{\text{Con.} \eqref{3.2}} \quad{}  \xi_{i}^{\*} = 0 \quad{}  \xrightarrow{\text{Con. (1.1)}} \quad{}  y^{(i)} ((w^{*})^{T}x^{(i)} + b^{\*}) \geq 1 \\)
* If \\(\alpha_{i}^{\*} = C \quad{}  \xrightarrow{\text{Eq. (5-11)}} \quad{}  \beta_{i}^{\*} = 0 \quad{}  \xrightarrow{\text{Con.} \eqref{1.1}), (1.2)}} \quad{}  y^{(i)} ((w^{\*})^{T}x^{(i)} + b^{\*}) \leq 1\\)
* If \\(0 < \alpha_{i}^{\*} < C \quad{}  \xrightarrow{\text{Eq. (5-11)}} \quad{}  \beta_{i}^{\*} \in (0,C) \quad{} \xrightarrow{\text{Con.} \eqref{3.2}} \quad{}  \xi_{i}^{\*} = 0 \quad{}  \xrightarrow{\text{Con. (3.1)}} \quad{}  y^{(i)} ((w^{\*})^{T}x^{(i)} + b^{\*}) = 1\\)
...
From a numerical stability standpoint, and in particular when taking into account the stopping criteria, the actual value of the margin may not be exactly equal to 1, so generally one averages over the \\(b^{\*}\\)'s of all support vectors when performing the calculation. If we denote \\(S = \\{i|a_i^{\*}>0\\}\\) as the set of support vectors, then
