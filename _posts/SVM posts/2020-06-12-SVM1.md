...
excerpt: "A thorough explanation of one of the best off-the-shelf machine learning algorithms: the support vector machine. The article discusses the primal- and dual problem, as well as the soft-margin SVM, kernelized SVM and SVO algorithm."
header: 
...
\textrm{subject to} && y^{(i)}(w^{T}x^{(i)} + b) \geq \hat{\gamma} & \forall i = 1, ..., m \\
\end{align}
