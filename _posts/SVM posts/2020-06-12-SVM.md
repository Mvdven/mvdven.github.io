...
\begin{align}
w_{(t+1)} = w_{(t)} - \eta \nabla_{w_{(t)}} \mathcal{C} \tag{2-6} \\
\end{align}

where \\(\eta\\) is defined as the learning rate and the gradient is given by,

\begin{align}
\nabla_{w} \mathcal{C} = \frac{1}{2}\frac{d}{dw}\|w\|^{2} + C \frac{d \mathcal{L}}{dw} \tag{2-7}
\end{align}

Clearly, because we cannot take the derivative at the point where \\(y^{(i)}(w^{T}x + b) = 1 \\) (see Fig. 4), the hinge loss function is non-differentiable. However, we can calculate the subderivative of the hinge loss function, as such

\begin{align}
\frac{d}{dw} \mathcal{L}(x^{(i)}, y^{(i)}) = \tag{2-8}
\begin{cases}
