...
where \\(\rho_{k} = \sum_{i}^{m} \alpha_i y^{(i)} K_{ik} +b\\) is the output of the kernelized decision function of Equation \eqref{4-5} with \\(x^{(k)}\\) as the input value. Note that \\(\upsilon_{k}\\) is independent of the update of the Lagrangian multipliers \\(\alpha_1\\) and \\(\alpha_2\\). In other words, we can express \\(\upsilon_{k}\\) using only \\(\alpha^{old}\\). This will come in handy later on.
...
Using Equation \eqref{5-7}, we can now identify and simplify the coefficient of the quadratic term \\(\alpha_{2}^{2}\\) as follows,
...
where \\(E_{k} = \upsilon_{k} - y^{(k)} \Leftrightarrow \sum_{i=1}^{m}a_{i}^{old} y^{(i)}K_{ik} + b - y^{(k)}\\) is the difference between the desired output \\(y_{k}\\) and the actual output based on the previous values of the alpha values. For the sake of clarity we supressed the "old" superscript untill the last line of derivation.
...
Note that the outcome of Equation \eqref{5-14} provides us with the unconstrained maximum value of \\(\alpha_{2}\\). We shall denote this value by \\(\alpha_{2}^{new}\\). To assure that \\(\alpha_{2}^{new}\\) satisfies the constraints of Equation \eqref{5-2}, we clip its value. 

Recall that the linear equality constraint in Equation \eqref{5-2} requires the Lagrange multipliers to lie on a diagonal line. Secondly, the bound constraints in Equation \eqref{5-2} requires the Lagrange multipliers to lie within the box \\([0,C] x [0,C]\\). Thus, both constraints imply that the constrained maximum of the objective function must lie on a diagonal line segment. Overall, there will be some lower-bound \\(L\\) and some upper-bound \\(U\\) on the permissible values for \\(\alpha_{2}\\) that will ensure that \\(\alpha_{1}\\) and \\(\alpha_{2}\\) lie on this diagonal line segment. In particular,
...
\begin{align}
a^{new, clipped}_2 = \tag{5-17} \label{5-17}
\begin{cases}
H &\quad{} \text{if } a_{2}^{new} \geq H \newline
a^{new}_{2} &\quad{} \text{if } L < a_{2}^{new} < H \newline
L &\quad{} \text{if } a_{2}^{new} \leq L \newline
\end{cases}
\end{align}
...
\begin{align}
E_{k}^{new} - E_{k}^{old} & =  (u_{k}^{new} - y^{(k)}) -  (u_{k}^{old} - y^{(k)}) \tag{5-18} \label{5-18} \newline
&\quad{} =  u_{k}^{new} - u_{k}^{old}\newline
&\quad{} = (\sum_{i=1}^{m} a_{i}^{new} y^{(i)} K_{ik} + b^{new}) - (\sum_{i=1}^{m} a_{i}^{old} y^{(i)} K_{ik} + b^{old}) \newline
&\quad{} = \sum_{i=1}^{m} a_{i}^{new} y^{(i)} K_{ik} - \sum_{i=1}^{m} a_{i}^{old} y^{(i)} K_{ik} + b^{new} - b^{old} \newline
&\quad{} = \sum_{i\neq 1,2}^{m} a_{i}^{new} y^{(i)} K_{ik} + \sum_{i = 1,2} a_{i}^{new} y^{(i)} K_{ik} - \sum_{i\neq 1,2}^{m} a_{i}^{old} y^{(i)} K_{ik} 
- \sum_{i = 1,2} a_{i}^{old} y^{(i)} K_{ik} + b^{new} - b^{old} \newline
&\quad{} = \sum_{i = 1,2} a_{i}^{new} y^{(i)} K_{ik}  - \sum_{i = 1,2} a_{i}^{old} y^{(i)} K_{ik} + b^{new} - b^{old} \newline
&\quad{} = (a_{1}^{new} y^{(1)} K_{1k} + a_{2}^{new} y^{(2)} K_{2k}) - (a_{1}^{old} y^{(1)} K_{1k} + a_{2}^{old} y^{(2)} K_{2k}) + b^{new} - b^{old} \newline
&\quad{} = (a_{1}^{new} - a_{1}^{old}) y^{(1)} K_{1k} + (a_{2}^{new, clipped} - a_{2}^{old}) y^{(2)} K_{2k} + b^{new} - b^{old}
\end{align}
...
\begin{align}
0 - E_{k}^{old} & = (a_{1}^{new} - a_{1}^{old}) y^{(1)} K_{1k} + (a_{2}^{new} - a_{2}^{old}) y^{(2)} K_{2k} + b^{new}_{k} - b^{old}_{k} \quad{} (k = 1 \text{or } 2) \tag{5-19} \label{5-19}\newline
