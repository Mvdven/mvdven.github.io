...
__Figure 4: Effect of outliers on maximum margin classifier with high \\(C\\) (left) and low \\(C\\) (right)__
...
\begin{align}
\sum_{i=1}^{m} a_i y^{(i)} = 0 \Leftrightarrow \alpha_1 y^{(1)} + \alpha_2 y^{(2)} = -\sum_{i \neq 1,2}^m \alpha_i y^{(i)} \tag{5-3} \label{5-3}
\end{align}
...
\begin{cases}
U &\quad{} \text{if } \alpha_{2}^{new} \geq U \newline
\alpha_{2}^{new} &\quad{} \text{if } L < \alpha_{2}^{new} < U \newline
...
\* _Note that_ \\(y^{(k)} E_{k} = 0 \Leftrightarrow y^{(k)} (\sum_{i=1}^{m} \alpha_{i} y^{(i)} K_{ik} + b - y^{(k)}) = 0 \Leftrightarrow y^{(k)} f(x^{(k)}) - 1 = 0 \Leftrightarrow y^{(k)} f(x^{(k)}) = 1\\). _This is exactly the KKT condition if_ \\(0 < a_{k} < C\\).
...
while iteration < maxiteration:
    ...
    for i in range(1, m+1):     # for each alpha_i
        ...
        for j in range (1, m+1): # for each alpha_j
            ...
            if eta >= 0:	# positive SOC, ambiguous whether there is a maximum
            	continue
