...
1. Primal feasibility: the primal constraints hold, i.e.
    ...
    1.1 \\(\quad{} y^{(i)} ((w^{\*})^{T}x^{(i)} + b^{\*}) \geq 1 - \xi_i^{\*}  \quad{} \forall i = 1, ..., m \tag{Con 1.1} \label{1.1}\\)
    
    1.2 \\(\quad{} \xi_i^{\*} \geq 0 \quad{} \forall i = 1, ..., m \tag{Con 1.2} \label{1.2} \\)


2. Dual feasibilitiy: the dual constraints hold, i.e.
    
    2.1 \\(\quad{} a^{\*} \geq 0 \tag{2.1} \label{2.1} \\)
    
    2.2 \\(\quad{} \beta^{\*} \geq 0 \tag{2.2} \label{2.2}\\)


3. Complimentary slackness, i.e.
    
    3.1 \\( \quad{} \alpha_{i}^{\*} \\{(1 - \xi_{i}^{\*}) - y^{(i)} ( (w^{\*})^{T}x^{(i)} + b^{\*}) \\} = 0 \quad{} \forall i = 1, ..., m \tag{3.1} \label{3.1} \\)
    
    3.2 \\( \quad{} \beta_{i}^{\*} ( \text{-} \xi_{i}^{\*} ) = 0 \quad{} \forall i = 1, ..., m \tag{3.2} \label{3.2} \\)

4. Stationarity: the gradient with respect to \\(w\\), \\(b\\) and \\(\xi\\) is zero, i.e.

    4.1 \\(\quad{} \frac{\delta}{\delta w}L(w^{\*}, b^{\*}, \xi^{\*}, \alpha^{\*}, \beta^{\*}) = 0 \tag{4.1} \label{4.1} \\)
    
    4.2 \\(\quad{} \frac{\delta}{\delta b}L(w^{\*}, b^{\*}, \xi^{\*}, \alpha^{\*}, \beta^{\*}) = 0 \tag{4.2} \label{4.2}\\)
    
    4.3 \\(\quad{} \frac{\delta}{\delta \xi}L(w^{\*}, b^{\*}, \xi^{\*}, \alpha^{\*}, \beta^{\*}) = 0 \tag{4.3} \label{4.3}\\)

From these conditions, we can extract some usefull information. In particular,

* If \\(\alpha_{i}^{\*} = 0 \quad{} \xrightarrow{\text{Eq.} \eqref{3-11}} \quad{}  \beta_{i}^{\*} = C \quad{} \xrightarrow{\text{Con.} \ref{3.2}} \quad{}  \xi_{i}^{\*} = 0 \quad{}  \xrightarrow{\text{Con.} \eqref{1.1}} \quad{}  y^{(i)} ((w^{*})^{T}x^{(i)} + b^{\*}) \geq 1 \\)
