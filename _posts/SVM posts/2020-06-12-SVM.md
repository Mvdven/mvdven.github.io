...
\tag{5-15} \label{5-15}
\begin{cases}
L = \max(0, \zeta - C) = \max(0, \alpha_{1} + \alpha_{2} - C) \newline
...
The python code for the main part of the SMO algorithm is listed below. There are several existing libraries designed specifically for solving the optimization problem(s) presented by SVMs such as LIBLINEAR/LIBSVM and CVXOPT. These libraries should be favored over the implementation below, which is meant to be a personal reference or educational resource.
...
__Figure 7: SVM with linear kernel__

![Figure 7: SVM with linear kernel](/images/end-result.png)
