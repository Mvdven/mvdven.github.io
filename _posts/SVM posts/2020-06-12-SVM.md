...
\begin{align} 
L (w, b, \xi, \alpha, \beta) = \underbrace{\frac{1}{2} \|w\|^{2} + C \sum_{i}^{m} \xi_{i}}_{\text{Primal objective}} + \underbrace{\sum_{i}^{m} \alpha_{i} [(1 - \xi_{i}) - y^{(i)} (w^{T} x^{(i)} + b)]}_{\text{First set of ineq. constraints}} + \underbrace{\sum_{i}^{m} \beta_{i} (-\xi_{i})}_{\text{Second set of ineq. constraints}} \tag{3-1} \label{3-1}
\end{align}
