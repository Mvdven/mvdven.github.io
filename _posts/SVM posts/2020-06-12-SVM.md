...
Note that, according to the decision rule of Equation (1-2), if \\(y^{(i)} = 1\\) then we need \\(f(x^{(i)})\\) to be large and positive in order for the prediction to be correct and confident (i.e. far away from the decision surface). Similarly, if \\(y^{(i)} = -1\\), we need \\(f(x^{(i)})\\) to be a large negative number for our prediction to be confident and correct. Overall, when \\(\hat{\gamma}^{(i)}\\) is large and positive this represents a correct and confident prediction. 
