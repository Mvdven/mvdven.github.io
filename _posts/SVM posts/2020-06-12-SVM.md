...
where \\(\rho_{k} = \sum_{i}^{m} \alpha_i y^{(i)} K_{ik} +b\\) is the output of the kernelized decision function of Equation \eqref{3-15} with \\(x^{(k)}\\) as the input value. Note that \\(\upsilon_{k}\\) is independent of the update of the Lagrangian multipliers \\(\alpha_1\\) and \\(\alpha_2\\). In other words, we can express \\(\upsilon_{k}\\) using only \\(\alpha^{old}\\). This will come in handy later on.
...
&\quad = \zeta + \alpha_2 (1 - s)  - \frac{1}{2} (\zeta - s \alpha_{2})(\zeta - s \alpha_{2}) K_{11} - \frac{1}{2} \alpha_{2}^{2} K_{22} - \zeta \alpha_{2} s K_{12} + \alpha_{2}^{2} s^{2} K_{12} \newline
