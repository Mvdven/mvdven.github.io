...
From Equation (2-1), it is worth noting that the constraint \\(y^{(i)} (w^{T}x^{(i)} + b) \geq 1 - \xi_i\\), which is equivalent to \\(\xi_i \geq 1-y^{(i)} (w^{T}x^{(i)} + b)\\), together with \\(\xi_i \geq 0\\), can be written more concisely. Particularly, because we are are minimizing \\(\xi_{(i)}\\), and the smallest value that satisfies the constraint occurs at equality,
...
Now, using the hinge loss function we can rewrite the cost function of Equation (2-3) as,
...
Noting that the derivative of a sum is equal to the sum of its derivatives, we can calculate the gradient of Equation (2-5) as follows,

\begin{align}
\nabla_{w} \mathcal{C} &= \frac{1}{2}\frac{d}{dw}\|w\|^{2} + C \sum_{i=1}^{m} \frac{d}{dw} {L}(x^{(i)}, y^{(i)}) \tag{2-7} \newline
&\quad{} = w  + C \sum_{\substack{i = 1 s.t. \newline  y^{(i)}f(x^{(i)}) <1}}^{m} y^{(i)}x^{(i)}
\end{align}
...
Note that the cost function of Equation (2-5) consists of the sum of two convex functions. Taken in mind that the sum of two convex functions is convex, we can apply gradient descent to find the global optimal parameter values.