...
excerpt: "A thorough explanation of the one of the best off-the-shelf machine learning algorithms: the support vector machine."
comments: true
mathjax: "true"
classes: wide
header:
...
<div class="notice--warning">
  <h4>Please note</h4>
  <p>It might take a couple of seconds before the mathematical notation is correctly loaded into your web browser.</p>
</div>
...
__Figure 5: Hinge loss function is piece wise linear__

![Figure 5: Hinge loss function is piece wise linear](/images/piece-wise-linear.png)
...
\begin{align}
\phi(x) = \begin{bmatrix} 1 \newline x_{1}^2 \newline x_{2}^2 \newline \sqrt{2}x_{1} \newline \sqrt{2}x_{2} \newline \sqrt{2}x_{1}x_{2}  \end{bmatrix} \quad \text{ and } \quad
...
On the other hand, if \\(s = -1\\) (Fig 7), i.e. the line is upward sloping, then \\(\alpha_1 + s\alpha_2 = \zeta \Rightarrow \alpha_2 = -\zeta + \alpha_1\\). Moreover,
...
\begin{align}
\alpha_{2}^{new, clipped} = \tag{5-17} \label{5-17}
\begin{cases}
...
Finally, we can use \\(\alpha_1^{new} = \alpha_1^{old} + s (\alpha_{2}^{old} - \alpha_{2}^{new \text{, } clipped})\\) from our discussion surrounding \eqref{5-4} to find the maximum of \\(\alpha_{1}\\).
...
& =  \rho_{k}^{new} - \rho_{k}^{old}\newline
& = (\sum_{i=1}^{m} \alpha_{i}^{new} y^{(i)} K_{ik} + b^{new}) - (\sum_{i=1}^{m} \alpha_{i}^{old} y^{(i)} K_{ik} + b^{old}) \newline
& = \sum_{i=1}^{m} a_{i}^{new} y^{(i)} K_{ik} - \sum_{i=1}^{m} a_{i}^{old} y^{(i)} K_{ik} + b^{new} - b^{old} \newline
...
& = \sum_{i = 1,2} a_{i}^{new} y^{(i)} K_{ik} - \sum_{i = 1,2} \alpha_{i}^{old} y^{(i)} K_{ik} + b^{new} - b^{old} \newline
& = (\alpha_{1}^{new} y^{(1)} K_{1k} + \alpha_{2}^{new} y^{(2)} K_{2k}) - (\alpha_{1}^{old} y^{(1)} K_{1k} + \alpha_{2}^{old} y^{(2)} K_{2k}) + b^{new} - b^{old} \newline
& = (a_{1}^{new} - \alpha_{1}^{old}) y^{(1)} K_{1k} + (\alpha_{2}^{new, clipped} - \alpha_{2}^{old}) y^{(2)} K_{2k} + b^{new} - b^{old} \newline
\end{align}
...
\begin{align}
0 - E_{k}^{old} & = (\alpha_{1}^{new} - \alpha_{1}^{old}) y^{(1)} K_{1k} + (\alpha_{2}^{new} - \alpha_{2}^{old}) y^{(2)} K_{2k} + b_{k}^{new} - b_{k}^{old} \quad{} (k = 1 \text{or } 2) \tag{5-19} \label{5-19}\newline
b_k^{new} & = - E_{k}^{old} - (a_{1}^{new} - a_{1}^{old}) y^{(1)} K_{1k} - (a_{2}^{new} - a_{2}^{old}) y^{(2)} K_{2k} + b_{k}^{old} \newline
\end{align}

* If both \\(\alpha_{1}\\) and \\(\alpha_{2}\\) are both at the bounds (well away from margin), then
anything between \\(b_{1}\\) and \\(b_{2}\\) works, so we set, \\(b = (b_{1} + b_{2})/2\\). 
